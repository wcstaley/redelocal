...
class GF_SendGrid extends GFAddOn {
	...
	public function maybe_send_email( $email, $message_format, $notification, $entry ) {
		...
		try {
			...
			$this->log_debug( __METHOD__ . "(): Notification (#{$notification['id']} - {$notification['name']}) for entry {$entry['id']} successfully sent by SendGrid; " . print_r( $sent_email, true ) );
			...
			$this->log_error( __METHOD__ . "(): Unable to send notification (#{$notification['id']} - {$notification['name']}) for entry {$entry['id']} with SendGrid; " . $e->getMessage() );
